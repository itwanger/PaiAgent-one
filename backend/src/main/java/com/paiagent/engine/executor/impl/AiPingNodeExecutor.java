...
@Slf4j
@Component
public class AiPingNodeExecutor implements NodeExecutor {
    ...
    public Map<String, Object> execute(WorkflowNode node, Map<String, Object> input) throws Exception {
        ...
        // 获取配置 - 兼容 apiUrl 和 baseUrl
        String apiUrl = (String) data.get("apiUrl");
        if (apiUrl == null || apiUrl.trim().isEmpty()) {
            apiUrl = (String) data.get("baseUrl");
        }
        ...
        log.info("AI Ping 节点配置 - API URL: {}, Model: {}, Temperature: {}", apiUrl, model, temperature);
        log.info("AI Ping 输入参数配置: {}", inputParams);
        log.info("AI Ping 输入数据: {}", input);
        ...
        // 构建 API 请求地址 - AI Ping 使用类似 OpenAI 的格式
        ...
        log.info("请求 AI Ping API: {}", fullApiUrl);
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                if (choices != null && !choices.isEmpty()) {
                    ...
                    // 如果没有 choices，可能 AI Ping 使用了不同的响应格式，尝试其他字段
                    if (responseJson.containsKey("response")) {
                        apiResponse = responseJson.getString("response");
                    } else {
                        apiResponse = "API 返回格式异常: " + response.getBody();
                    }
                }
            } else {
                apiResponse = "API 调用失败，状态码: " + response.getStatusCode() + ", 响应: " + response.getBody();
            }
        } catch (Exception e) {
            log.error("调用 AI Ping API 失败", e);
        ...
        // 获取 token 使用量（如果有的话）
        if (apiResponse != null && !apiResponse.startsWith("调用 API 失败") && !apiResponse.startsWith("API 返回格式异常")) {
            output.put("tokens", random.nextInt(100) + 50); // 随机生成 token 数 50-150
        } else {
            output.put("tokens", 0);
        }

        log.info("AI Ping 节点输出: {}", output);
